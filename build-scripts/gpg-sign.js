  ...
  return new Promise((resolve, reject) => {
    const options = {
      ...
      headers: {
        ...
        "User-Agent": "ROSI-LTS-Release-Script",
    ...
    try {
      ...
      if (createError.statusCode === 422) {
        ...
        if (Array.isArray(releases)) {
          ...
          if (release) {
            console.log(
              "   Found release after retry: " + (release.name || TAG_NAME),
            );
            return release;
          }
        }
...
async function main() {
  ...
  console.log("GPG Sign & Upload - ROSI-LTS " + VERSION);
  ...
  if (!GH_TOKEN) {
    console.warn(
      "⚠ WARN: GH_TOKEN not set - signatures will not be uploaded to GitHub",
    );
  }
